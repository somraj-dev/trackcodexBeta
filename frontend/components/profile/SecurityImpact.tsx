...
const SecurityImpact: React.FC<Props> = ({ profile }) => {
  const navigate = useNavigate();

  const metrics = profile?.skillMetrics;
  const vulnsFixed = metrics?.vulnerabilitiesFixed ?? null;
  const securityIssues = metrics?.securityIssuesReported ?? null;
  const prReviews = metrics?.prReviewsGiven ?? null;

  const hasAnyData = vulnsFixed !== null || securityIssues !== null || prReviews !== null;
      ...
      {hasAnyData ? (
        <>
          ...
          >
            ...
            <span className="text-[13px] font-bold text-gh-text-secondary group-hover/btn:text-gh-text transition-colors">
              {prReviews !== null ? `${prReviews} PR Reviews Completed` : "No reviews yet"}
            </span>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 relative z-10">
          <div className="size-16 rounded-full border-2 border-dashed border-gh-border flex items-center justify-center">
            <span className="material-symbols-outlined text-gh-text-secondary !text-[28px]">
              security
            </span>
          </div>
          <p className="text-[13px] text-gh-text-secondary font-medium text-center max-w-[180px]">
            Security metrics will appear once you start fixing vulnerabilities and reviewing PRs.
          </p>
          <button
            onClick={() => navigate("/activity")}
