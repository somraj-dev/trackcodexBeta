...
const CommunityLeftSidebar: React.FC<CommunityLeftSidebarProps> = ({
    ...
    return (
        ...
        >
            {/* Header & Toggle */}
            <div className={`p-4 flex items-center justify-between ${isCollapsed ? 'justify-center border-b border-[#1A1A1A] pb-4' : ''}`}>
                {!isCollapsed && (
                ...
                <button
                    ...
                    className={`size-8 rounded-full border border-[#333333] hover:bg-[#1A1A1A] flex items-center justify-center text-[#A1A1AA] transition-colors ${isCollapsed ? 'mt-2' : ''}`}
                                ...
                                {communities.map(community => (
                                    <div key={community.id} className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-[#1A1A1B] cursor-pointer group">
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            <img src={community.avatar || `https://ui-avatars.com/api/?name=${community.name}`} className="size-6 rounded-full" alt="" />
                                            <span className="text-sm text-[#D7DADC] truncate">{community.name}</span>
                                        </div>
                                        <button
                                            onClick={(e) => {
                                            ...
                                            className={`material-symbols-outlined !text-[18px] transition-colors ${starredIds.has(community.id) ? 'text-yellow-500 filled' : 'text-[#575757] group-hover:text-[#A1A1AA]'
                                                }`}
                                        >
                                            star
                                        </button>
            ...
            {!isCollapsed && (
                <div className="p-4 border-t border-[#1A1A1A] transition-all duration-300">
                    {user ? (
