...
const Login = () => {
  ...
  const handleGithubLogin = async () => {
    ...
    try {
      if (!isFirebaseConfigured) {
        throw new Error("Firebase Authentication is not configured. TrackCodex login unavailable.");
      }
      ...
      if (error.code !== 'auth/popup-closed-by-user' && error.code !== 'auth/cancelled-popup-request') {
        console.error(error);
        setError(error.message || "Failed to initialize TrackCodex login");
      }
    } finally {
      setIsLoading(false);
    }
